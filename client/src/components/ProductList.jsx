...
export const ProductList = ({ onEditProduct, onAddProduct, presetFilter }) => {
  ...
  const [activePresetLabel, setActivePresetLabel] = useState('All Products');

  const toNumber = (value) => Number(value ?? 0);

  useEffect(() => {
    setActivePresetKind(presetFilter?.kind ?? 'all');
    setActivePresetLabel(presetFilter?.label ?? 'All Products');
  ...
  const getStockStatus = (stock, minStock) => {
    ...
    if (stock <= minStock) return { color: 'text-orange-600', bg: 'bg-orange-100', label: 'Low Stock' };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 dark:text-slate-50">Products</h1>
          <p className="text-gray-500 dark:text-gray-300 mt-1">Manage your inventory efficiently</p>
        </div>
        <button
      ...
      {activePresetKind !== 'all' && (
        <div className="card-lg p-4 border-l-4 border-primary-500 bg-gradient-to-r from-primary-50 to-primary-100 dark:from-slate-800 dark:to-slate-700">
          <div className="flex items-center justify-between">
            <p className="font-medium text-primary-900 dark:text-primary-300">
              📋 Showing: <span className="font-bold">{activePresetLabel}</span>
            </p>
            <button
              type="button"
              onClick={() => {
                setActivePresetKind('all');
                setActivePresetLabel('All Products');
              }}
      ...
      <div className="card-lg p-6">
        ...
        <div className="mt-4 flex items-center space-x-2 text-sm font-medium text-gray-600 dark:text-gray-400">
          ...
          <span>
            Displaying <span className="text-primary-500">{filteredProducts.length}</span> of <span className="text-primary-500">{products.length}</span> products
          </span>
        </div>
      </div>
              ...
              <div className="px-4 pt-4 pb-1 flex flex-col flex-1">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center justify-between ">
                    <div className="flex items-center space-x-2">
                      <Package className="h-5 w-5 text-gray-400 dark:text-gray-500" />
                      <span className="text-md font-medium text-gray-600 dark:text-gray-400">In Stock</span>
                    </div>
                    <div className="flex items-center space-x-2">
                      <span className="text-2xl font-bold text-gray-900 dark:text-white">{effectiveStock}</span>
                      <span className={`px-3 py-1 pt-1.5 rounded-full text-xs font-semibold ${stockStatus.bg} ${stockStatus.color}`}>
                  ...
                  <div>
                    <div className="flex items-center justify-between">
                      <span className="text-md text-gray-600 dark:text-gray-200">Selling Price</span>
                      <span className="text-2xl font-bold text-primary-600 dark:text-primary-500">₨ {price.toLocaleString()}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between border-t border-gray-200 dark:border-gray-500">
                      <span className="text-md font-medium text-gray-700 dark:text-gray-300">Profit Margin</span>
                      <span className={`text-lg font-bold ${profitMargin > 20 ? 'text-green-600' : profitMargin > 10 ? 'text-yellow-600' : 'text-orange-600'}`}>
                        {profitMargin}%
                      </span>
                    </div>
                  ...
                  {/* Batch Information */}
                  <div className="mt-2">
                    <p className="text-md font-semibold text-gray-600 dark:text-gray-200 uppercase tracking-wide">Batch Information</p>
                    <div className="flex items-center justify-between text-md">
                      <span className="text-gray-600 dark:text-gray-200">Batches:</span>
                      <span className="font-semibold text-gray-900 dark:text-gray-200">{batchSummaryByProduct[product.id]?.count ?? 0}</span>
                    </div>
                    {batchSummaryByProduct[product.id]?.batches?.length > 0 && (
                      <div className="text-md">
                        {batchSummaryByProduct[product.id].batches.slice(0, 1).map((batch) => (
                          <div key={batch.id} className="flex items-center justify-between gap-2">
                            <span className="font-medium text-gray-700 dark:text-gray-300">
                              {batch.batchNumber}{batch.vendorName || batch.vendorName ? ` (${batch.vendorName || batch.vendorName})` : ''}
                            </span>
                            <span className="text-gray-600 dark:text-gray-200">
                              {batch.quantity} @ ₨{Number(batch.unitPrice || 0).toFixed(0)}
                            </span>
